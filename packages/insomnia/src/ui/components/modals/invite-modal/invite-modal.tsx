...
const usePagination = (permissionRef: MutableRefObject<Record<Permission, boolean>>) => {
  ...
  const queryMembers = useCallback(async (orgId: string, page: number) => {
    ...
    console.log('queryString', queryStringRef.current);
    const {
    ...
    const pendingMembers = permissionRef.current['read:invitation']
      ...
      : [];
    console.log('acceptedMembersInCurrentPage', acceptedMembersInCurrentPage, 'acceptedMemberCount', acceptedMemberCount);
  ...
  console.log('queryString1', queryStringRef.current);

  const lastPageIdx = Math.max(0, Math.ceil(totalMemberCount / PAGE_SIZE) - 1);

  console.log('currentPage', currentPage, 'totalMemberCount', totalMemberCount, 'lastPageIdx', lastPageIdx);
...
const InviteModal: FC<{
  ...
  console.log('hasNextPage', hasNextPage, 'hasPrevPage', hasPrevPage);
  ...
  return (
    ...
    >
      <Modal className="w-[520px] rounded-md border border-solid border-[--hl-sm] p-[32px] max-h-full bg-[--color-bg] text-[--color-font]">
        <Dialog className="outline-none relative">
          {({ close }) => (<>
            ...
            <div className='flex justify-between leading-[24px] mb-[16px]'>
              ...
              <Group
                className="relative w-[173px] bg-[#343434] group/search py-[4px] pl-[12px] rounded"
                isDisabled={loading}
                onClick={() => {
                  ...
                  {member => {
                    ...
                    return ((
                      <ListBoxItem
                        ...
                        textValue={textValue}
                        className='flex justify-between outline-none gap-[16px] leading-[19px]'
                      >
